...
- 認識調整
- ルール設定
- かんたん設定で、90度/180度/それ以上のカーブ継続を別々に選択
- 別画面のかんたん設定ウィザード（1画面1判断）
- かんたん設定内で2つ目・3つ目のルールを追加可能
- かんたん設定中は専用ルールセットだけを走行に反映し、過去ルールとの重複発火を防止
- 目的位置の手前を起点にした時間差アクション設定
- 電圧補正設定
- ラップタイム確認
- 停止後ログ確認
...
3. 展開したフォルダ内の `START_HERE_Flash_Mini4AI.cmd` を実行します。
...
スタンドアローン起動で使いたい設定は、必ずWebアプリ左メニューの **「本体へ保存」** を押してから使用してください。

Webアプリでは、誤操作を避けるため「走行開始」「停止」とは離した左メニューに **本体へ保存** を配置しています。保存時には確認画面で「はい / いいえ」を選択します。
...
Webアプリには、通常の詳細UIとは別に、1画面1判断で進む **かんたん設定** があります。
目的位置、起点セクション、発動回数、待機時間、ブレーキ/低出力を順番に設定できます。

走行後に「早すぎた」「遅すぎた」「発動しなかった」「違う場所でも発動した」を選ぶと、修正すべき画面へ戻ります。
このとき、改善アドバイスを画面上部に表示し続けるため、何を直すべきか確認しながら調整できます。

1つ目のルールが決まった後も、かんたん設定のまま2つ目・3つ目のルールを追加できます。
...
かんたん設定では単発の左C/右Cを初期状態で非表示にし、誤発動しにくい左/右カーブ継続を基本にしています。
| ファームウェア | v3.58-r10 |
...
目的の場所で止める、または減速する場合は、**その場所そのものを認識してからブレーキするのではなく、手前の特徴的なセクションを起点にして時間差でアクション**します。

例として、ウェーブで止まりたい場合、ウェーブを認識してからブレーキすると遅れることがあります。ウェーブの手前にある特徴的なセクションを起点にし、待機msで位置を合わせてからブレーキ/低出力を出します。

最初は1〜2セグメントの短い条件で作り、同じ起点が複数回出る場合は、奇数回・偶数回・N回のみの発動条件を使います。

Webアプリの **かんたん設定** は通常画面とは別画面で開き、1画面につき1つの判断だけを表示します。
最後の接続前には、設定したルールを自然文で確認できます。
走行後は「発動しない」「早すぎる」「遅すぎる」「余計な場所でも出る」から選ぶと、次に触るべき設定を1つに絞って提案します。

詳細は [かんたん設定ウィザード](docs/guided_setup.md) を参照してください。
...
- [かんたん設定ウィザード](docs/guided_setup.md)
